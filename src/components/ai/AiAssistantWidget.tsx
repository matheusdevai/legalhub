        ...
        <div className="fixed bottom-[164px] right-6 z-50 w-[360px] bg-white dark:bg-dark-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-dark-700 flex flex-col overflow-hidden animate-scale-in">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {SUGESTOES.map(s => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="text-xs px-2.5 py-1.5 rounded-full border border-primary-200 dark:border-primary-800 text-primary-700 dark:text-primary-300 hover:bg-primary-50 dark:hover:bg-primary-900/30 transition-colors"
                    >
                      {s}
                    </button>
                  ))}
          ...
          <div className="flex-shrink-0 border-t border-gray-100 dark:border-dark-700 p-3 flex items-center gap-2">
            ...
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-xl bg-primary-600 hover:bg-primary-700 text-white disabled:opacity-50 transition-colors flex-shrink-0"
            >
      ...
      <button
        ...
        className={cn(
          ...
          open
            ...
            : 'bg-gradient-to-br from-primary-700 via-primary-600 to-primary-500 hover:scale-105 shadow-button-lg',
        )}
        title="Copiloto Lawfy"
      >
        {open ? <ChevronDown className="w-5 h-5 text-white" /> : <Sparkles className="w-6 h-6 text-white" />}
        {!open && <span className="absolute inset-0 rounded-full bg-primary-500 opacity-30 animate-ping" />}
      </button>
    </>
  )
}
