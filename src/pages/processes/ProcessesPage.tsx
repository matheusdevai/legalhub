...
import { ProcessFormFields, FASES, CONTINGENCIAMENTOS } from './ProcessFormFields'
...
// Lista própria do painel de edição (ViewPanel) — mantida separada da lista mais
// completa usada pelos dois fluxos de criação (ProcessFormFields), pra não alterar
// a exibição de processos previdenciários já cadastrados com os valores antigos.
const VIEWPANEL_TIPOS_ACAO: Record<string, string[]> = {
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-end sm:justify-end bg-black/40 backdrop-blur-sm">
          <div className="w-full sm:w-[440px] h-full sm:h-screen bg-white dark:bg-dark-800 flex flex-col shadow-2xl overflow-hidden">
            {/* Header — mesmo tratamento visual (banner com gradiente) do passo "Novo processo" da conclusão de atividade em TasksPage */}
            <div className="relative overflow-hidden flex-shrink-0 bg-gradient-to-br from-emerald-600 via-green-600 to-emerald-500 text-white px-5 py-4">
              ...
              <div className="absolute right-3 top-3 opacity-20"><Briefcase className="w-16 h-16" /></div>
              <div className="relative flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-white/80 font-medium mb-0.5 uppercase tracking-wider">Processo</p>
                  <h3 className="text-base font-bold leading-snug pr-6 line-clamp-2">{editId ? 'Editar processo' : 'Criar novo processo'}</h3>
                </div>
                <button
                  onClick={() => { setModalOpen(false); setCpfSearch('') }}
            ...
            {/* Scrollable body — campos compartilhados com o passo "Novo processo" da conclusão de atividade (ProcessFormFields) */}
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              ...
              {(VIEWPANEL_TIPOS_ACAO[form.area] || []).map(t => <option key={t} value={t}>{t.toUpperCase()}</option>)}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
          </div>
        </div>
