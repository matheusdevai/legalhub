...
- **Sincronização OAB (CNJ/PJe)** → ao trocar a seccional (UF), `swapSeccionalTribunal()` em `OabSyncModal.tsx` troca o TJ da UF anterior pelo novo (nunca deixa o tribunal antigo "grudado" na lista). PJe (`sync-pje`) usa o DJEN/Comunica PJe (API pública gratuita do CNJ, `https://comunicaapi.pje.jus.br/api/v1/comunicacao`, sem autenticação) filtrando só por OAB+UF — cobre tribunais estaduais e federais automaticamente, sem exigir login/senha do usuário nem seleção manual de tribunal (diferente do CNJ/DataJud, que ainda precisa de tribunal selecionado). Limitação conhecida: só aparecem comunicações publicadas via Diário de Justiça Eletrônico Nacional — cobre a grande maioria dos tribunais (Resolução CNJ 455/2022 tornou isso obrigatório, TJSP incluso mesmo usando e-SAJ como sistema processual), mas não há garantia de 100% de cobertura de todo tipo de ato de todo tribunal.
- **Intimações recebidas (`PublicacoesPage.tsx`)** → layout master-detail: lista compacta (`IntimacaoListItem`) + painel de leitura (`IntimacaoReadingPanel`) com o texto integral (`movimento.teor`, gravado pela `sync-pje`/DJEN — não existe para movimentos vindos do CNJ/DataJud, que caem no fallback do rótulo curto + `complementosTabelados`). Abaixo do breakpoint `lg`, selecionar um item substitui a lista pelo painel inteiro (botão "Voltar"), sem dividir a tela em colunas apertadas. CTA "Criar tarefa de prazo" é um botão primário sempre visível no painel (não mais um ícone escondido no hover).
- **Prazo de intimação → Agenda** → o fluxo "Criar tarefa de prazo" cria uma `tasks` (`type: 'deadline'`) que o trigger `trg_sync_task_calendar_event`/`sync_task_calendar_event()` sincroniza automaticamente para `calendar_events` (exige `assigned_to` preenchido — cai por `trg_auto_assign_task` quando a task nasce sem responsável). A função propaga `tasks.type` para `calendar_events.type` quando compatível (`hearing`/`deadline`/`meeting`), com fallback pra `'task'` nos demais casos (`document`/`custom`/nulo) — `calendar_events.type` só aceita `hearing|deadline|meeting|task` via CHECK constraint, então o fallback é obrigatório pra não violar o constraint. Antes da migration `20260910120000_fix_sync_task_calendar_event_type`, o type gravado era sempre `'task'` fixo, então prazos apareciam na Agenda como "Tarefa" genérica em vez de "Prazo".
...
Nota histórica: existia uma segunda Edge Function mais antiga e mais simples (`ai-assistant`, "Copiloto Lawfy" — 6 tools só leitura, sem confirmação, sem histórico) que `AiCopilotoTab.tsx`/`AiAssistantWidget.tsx` usavam antes da unificação. O código-fonte foi removido do repositório (não existe mais `supabase/functions/ai-assistant/` aqui) e nenhum componente chama mais esse endpoint (confirmado por grep) — mas a function **continua deployada no projeto Supabase de produção**, porque não há ferramenta de delete/disable de Edge Function disponível via Supabase MCP nem CLI autenticada localmente neste ambiente. Em vez de ficar com o código antigo funcional e órfão (bypassando a confirmação/auditoria do fluxo novo), ela foi esvaziada: a versão publicada em produção só responde `410 Gone` (`"Este endpoint foi descontinuado. Use ai-assistant-chat."`), sem executar nenhuma query nem chamar o Gemini. Se uma ferramenta de delete de Edge Function ficar disponível no futuro, `ai-assistant` pode ser removida de vez do projeto.
...
| `lawfy-sync-processos-oab` | 5h | Sincroniza processos via CNJ (DataJud) + PJe (DJEN) + Escavador + JusBrasil, para **todo** profile com `oab_number`/`oab_seccional` preenchidos (qualquer tenant, role ≠ client). Chama a Edge Function `cron-sync-processes`, que reimplementa a lógica de `sync-cnj`/`sync-pje`/`sync-escavador`/`sync-jusbrasil` de forma standalone (sem exigir JWT de usuário — autentica por segredo fixo no header `x-cron-secret`, checado no início do handler). PJe entrou no cron a partir da migração para DJEN (antes ficava de fora por exigir CPF+senha por chamada via MNI/SOAP; o DJEN só precisa de OAB+UF, então é seguro rodar automaticamente) — janela curta de 2 dias por execução (a sincronização manual no `OabSyncModal.tsx` usa 90 dias). Isolamento por tenant garantido porque cada iteração do loop processa 1 profile por vez e toda leitura/escrita usa `profile.tenant_id` daquele profile — nunca mistura dados entre escritórios. Tribunais pesquisados no CNJ (DataJud) vêm de `profiles.oab_tribunais` (persistido pelo `OabSyncModal.tsx` a cada sync manual); se vazio, usa o TJ da seccional do usuário — PJe/DJEN não usa essa lista (filtra só por OAB+UF, nacional). Gera notificação (`type: 'system'`, link `/processos?auto_sync=1`) se importou/atualizou algo, deduplicada por dia. |
...
- `supabase/config.toml` → `auth.additional_redirect_urls` corrigido para `https://legalhubgestor.vercel.app/**` (cobre `/reset-password` e `/dashboard`, os únicos `redirectTo` gerados pelo app, em `Login.tsx`). Aplicado também no projeto live via `supabase config push` — **editar só o arquivo local NÃO muda o Auth do projeto em produção**, é preciso rodar o push (ou editar manualmente em Dashboard > Authentication > URL Configuration). Antes do fix, o allowlist live nem tinha `lawfy-saas` nem o domínio real — só URLs de preview do Lovable (`*.lovableproject.com`/`*.lovable.app`), então o redirect de auth em produção estava quebrado. ⚠️ `supabase config push` reescreve TODA a seção `[auth]` — como o `config.toml` não declarava `[auth.email]`/`[auth.mfa.totp]`, o primeiro push acidentalmente desligou confirmação de e-mail obrigatória e MFA TOTP ao vivo (default do CLI, não o que estava configurado no Dashboard). Corrigido restaurando esses valores explicitamente no arquivo antes do push definitivo — qualquer push futuro deve manter essas seções em sincronia com o Dashboard antes de rodar, sob risco de repetir o problema. `site_url` no arquivo (`http://localhost:3000`) reflete o valor que já estava live (não é o dev port real do Vite, 5173 — mismatch pré-existente, fora do escopo deste fix).
