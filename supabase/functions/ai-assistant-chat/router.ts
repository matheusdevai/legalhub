...
import {
  ...
  proporCriarTarefa, proporCriarLembrete,
  type CallerProfile,
} from './tools.ts'
import {
...
// propor_criar_tarefa/propor_criar_lembrete (milestone 3) NUNCA gravam nada
// sozinhas — devolvem uma ProposedAction que index.ts loga com status
// 'proposed' e devolve ao frontend como card de confirmação; a gravação de
// fato só acontece via confirm_action, tratado à parte em tools.ts
// (processConfirmAction), nunca por aqui.
export const WRITE_TOOL_NAMES = new Set<ToolName>(['propor_criar_tarefa', 'propor_criar_lembrete'])
